...
export class MapLayoutInformation extends PrimitiveTypeInformation {
   ...
   protected override consumeInternal(
      ...
      if (isImplicit) {
         const element = context.childWithEncoding(
            "map.element",
            additionalProperties,
         );
         this.set("element", element[0]);
      } else {
         const element = this.getValueChildWithEncodingExplicit(
            context,
            additionalProperties,
         );
      ...
      if (isImplicit) {
         const propertyNames = consumer.getProperty<SchemaDefinition>(
         ...
         if (propertyNames.hasValue()) {
            ...
            encoding = new StringEncodingInformation(new StringInformation());

         this.set("key", encoding);
      } else {
         const encoding = this.getKeyChildWithEncodingExplicit(
            context,
            additionalProperties,
         );
         this.set("key", encoding);
      ...
      if (
         ...
         const minimum =
            additionalProperties.getProperty<SchemaDefinition>("minimum");
         const maximum =
            additionalProperties.getProperty<SchemaDefinition>("maximum");

         if (minimum.hasValue())
            Reflect.set(
               this.element.metadata,
               "min_value",
               minimum.extract("number"),
            );
      ...
      const maxProperties =
         consumer.getProperty<SchemaDefinition>("maxProperties");
      if (maxProperties.hasValue()) {
         Reflect.set(
            this.metadata,
            "max_properties",
            maxProperties.extract("number"),
         );
      }
   }
   protected isImplicit(additionalProperties: Consumer): boolean {
      ...
      if (type.hasValue())
         if (type.extract("string") === "object") {
            const properties =
               additionalProperties.getProperty<SchemaDefinition>("properties");
