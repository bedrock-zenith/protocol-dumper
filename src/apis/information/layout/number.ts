...
> extends EncodingInformation {
   ...
   public override consumeInternal(context: Context, consumer: Consumer): void {
      ...
      if (minimum.hasValue())
         Reflect.set(this.metadata, "min_value", minimum.extract("number"));

      if (maximum.hasValue())
         Reflect.set(this.metadata, "max_value", maximum.extract("number"));
   }
}
...
export class IntegerEncodingInformation extends NumberEncodingInformation<Integer> {
   ...
   public override consumeInternal(context: Context, consumer: Consumer): void {
      ...
      const backing = consumer.getProperty<SchemaDefinition>(
         SCHEMA_KEYS.UNDERLYING_TYPE,
      );
      if (backing.hasValue()) {
      ...
      if (options.hasValue()) {
         if (
            INTEGER_SPECIAL_CASES.includes(
               context.name as (typeof INTEGER_SPECIAL_CASES)[number],
            )
         ) {
            void options.extractIncludes(
               "string",
...
export class Utils {
   ...
   public static getEncodingBits(options: Consumer): EncodingBits {
      const hasCompression = options.extractIncludes(
         "string",
         SERIALIZATION_OPTIONS.COMPRESSION,
      );
      const notCompressed = options.extractIncludes(
         "string",
         SERIALIZATION_OPTIONS.NO_SIZE_COMPRESSION,
      );
      const bigEndian = options.extractIncludes(
         "string",
         SERIALIZATION_OPTIONS.BIG_ENDIAN,
      );
      const littleEndian = options.extractIncludes(
         "string",
         SERIALIZATION_OPTIONS.LITTLE_ENDIAN,
      );
      return {
