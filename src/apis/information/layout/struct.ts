...
export class StructLayoutInformation extends BindTypeInformation {
   ...
   public override getTypeKey(): string {
      return (
         super.getTypeKey() +
         this.fields.map((_) => _.name + _.type.getLayoutKey() + _.is_constant)
      );
   }
      ...
      return (
         ...
         this.fields.map(
            (_) =>
               _.name +
               _.type.getLayoutKey() +
               JSON.stringify(_.metadata ?? null),
         )
      );
   }
   public override consumeInternal(context: Context, consumer: Consumer): void {
      ...
      for (const key of keys) {
         ...
         const index = consumer
            .getProperty<SchemaField>(SCHEMA_KEYS.ORDINAL_INDEX)
            .extract("number");
         ...
         if (
            consumer.hasProperty<SchemaField>("const") ||
            consumer.hasProperty<SchemaField>("default")
         ) {
