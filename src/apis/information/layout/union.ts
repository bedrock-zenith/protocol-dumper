...
import { IntegerEncodingInformation, IntegerInformation, Utils } from './number';
...
export class UnionLayoutInformation extends BindTypeInformation {
    ...
    public static createBackingEnum(name: string, values: string[]): EnumLayoutInformation {
    ...
    public override getData(data: object, scope: DataScope): void {
        ...
        if (scope === 'layout') {
            ...
            Reflect.set(data, 'backing_enum', this.backing.createData('reference'));
        }
    }
    ...
    protected consumeFields(context: Context, consumer: Consumer): EncodingInformation[] {
        const unionFields = consumer.getProperty<SchemaDefinition>(SCHEMA_KEYS.ONE_OF);
        ...
        for (const field of unionFields.getIterator()) {
            ...
            const ordinal = field.getProperty<SchemaField>(SCHEMA_KEYS.ORDINAL_INDEX);
            if (ordinal.hasValue()) {
                const index = ordinal.extract('number');
                if (index !== fields.length)
                    throw new ReferenceError('ordinal index has to mach its true index, did mojang fckp?');
            }
            fields.push(type[0]);
        }
    ...
    protected resolveBackingEnum(fields: EncodingInformation[], context: Context): void {
        const enumLayouts = fields.map((field) => {
            ...
            if (firstField.type.layout instanceof OptionalInformation) return undefined;
            if (!(firstField.type.layout instanceof EnumLayoutInformation)) return undefined;

            const metadata = firstField.metadata as { constant_value?: unknown } | undefined;
        ...
        const backing =
            ...
            enumLayouts.every(
                (enumLayout) =>
                    ...
                    Object.keys(enumLayout.fields).length === Object.keys(firstEnum.fields).length
            )
                ? firstEnum
                : undefined;
        ...
        const constants = fields.map((field) => {
            const layout = field.layout;
            if (!(layout instanceof StructLayoutInformation)) return undefined;
            const metadata = layout.fields[0]?.metadata as { constant_value?: unknown } | undefined;
            return metadata?.constant_value;
        });
        const hasConstants =
            constants.length > 0 &&
            constants.every((value) => value !== undefined) &&
            new Set(constants.map((value) => String(value))).size === constants.length;
        const values = hasConstants
            ? constants.map((value) => String(value))
            : fields.map((field, index) => `${field.layout.name}${index}`);
...
export class UnionEncodingInformation extends EncodingInformation {
    ...
    public constructor(layout: UnionLayoutInformation) {
        super(layout);
        this.control = new IntegerEncodingInformation(new IntegerInformation('u32'), 'union');
    }
    public override getData(data: object, scope: DataScope): void {
        super.getData(data, scope);
        if (scope === 'layout' || scope === 'field') {
            ...
            Reflect.set(data, 'control_encoding', this.control.createData('field'));
        }
    }
    public override getKey(builder: KeyBuilder, scope: DataScope): void {
        super.getKey(builder, scope);
        if (scope === 'layout' || scope === 'field') this.control.getKey(builder, 'field');
    }
    protected createControlEncoding(type: string): EncodingInformation {
        ...
        return new IntegerEncodingInformation(new IntegerInformation(int), 'union');
    }
    protected override consumeInternal(context: Context, consumer: Consumer): void {
        const controlType =
            consumer.getProperty<SchemaDefinition>('x-control-value-type').extractOptional('string') ??
            'uint32';
