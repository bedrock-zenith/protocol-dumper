...
export class Consumer {
    public static readonly NULL = new Consumer(null);
    public readonly type: ConsumerKey;
    public readonly data: any;
    private is_consumed: boolean;
    private constructor(data: any) {
        this.data = data;
        this.type = new.target.typeOf(data);
        this.is_consumed = this.type === "null";
    }
    ...
    public discard(): boolean {
        const discarded = !this.isConsumed();
        this.is_consumed = true;
        return discarded;
    }
    ...
    public getKeys(): string[] {
        ...
        return Reflect.ownKeys(this.data)
            .filter(
                (_) =>
                    typeof _ === "string" &&
                    !(this.data[_] as Consumer).isConsumed(),
            )
            .map(String);
    }
    ...
    public at(index: number): Consumer {
        this.assert("array");
        return (this.data as Array<Consumer>)[index] ?? Consumer.NULL;
    }
    ...
    public isConsumed(): boolean {
        return (
            ...
            (this.type === "array" &&
                !(this.data as Array<Consumer>).some((_) => !_.isConsumed()))
        );
    }
        ...
        if (this.type === "object") {
            return Object.fromEntries(
                Object.entries(this.data)
                    .filter((_) => !(_[1] as Consumer).isConsumed())
                    .map(([key, value]) => [
                        key,
