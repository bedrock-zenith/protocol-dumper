...
export class Context {
   ...
   public child(
      name: string,
      consumer: Consumer,
      mode: "consumeWithEncoding" | "consume",
   ): BaseComponent {
      const context = this.chain(name);
      let type: BaseComponent = this.transformer.resolveDataComponent(
         context,
         consumer,
      );
      ...
      if (
         this.parent &&
         type.is(EnumComponent, StructComponent, UnionComponent)
      ) {
         type = this.transformer.registerNewType(
            type as DataComponent,
            this.getFullName(),
         );
      }
      if (mode === "consumeWithEncoding")
         type = type.withEncoding(context, consumer);
