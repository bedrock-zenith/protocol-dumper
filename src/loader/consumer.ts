...
export class Consumer {
    ...
    public extractIncludes<K extends PrimitiveKey>(key: K, value: ConsumerMap[K]): boolean {
        return (this.data as Array<Consumer>).some((_) => {
            if (_.type === key && _.getValue(key) === value) return (_.extract(key), true);
            return false;
        });
    }
    ...
    public extractOptional<K extends 'string' | 'boolean' | 'number'>(key: K): ConsumerMap[K] | null {
    ...
    public expect(): this {
        if (this.type === null) throw new ReferenceError('Nullable is not expected');
        return this;
    }
    ...
    public getKeys(): string[] {
        ...
        return Reflect.ownKeys(this.data)
            .filter((_) => typeof _ === 'string' && !(this.data[_] as Consumer).getIsConsumed())
            .map(String);
    }
    ...
    public extractMany<T extends Record<string, 'string' | 'boolean' | 'number'>>(
        map: T
    ): { [K in keyof T]: ConsumerMap[T[K]] } {
    ...
    public extractManyOptional<T extends Record<string, 'string' | 'boolean' | 'number'>>(
        map: T
    ): { [K in keyof T]: ConsumerMap[T[K]] | null } {
        const record: Record<string, any> = {};
        for (const key of Object.getOwnPropertyNames(map)) {
            record[key] = this.getProperty(key).extractOptional(map[key as keyof T]);
        }
    ...
    protected assert(type: ConsumerKey): void {
        if (this.type != type) throw new TypeError(`Expected ${type}, got ${this.type}`);
    }
    ...
    public getIsConsumed(): boolean {
        return (
            ...
            (this.type === 'array' && !(this.data as Array<Consumer>).some((_) => !_.getIsConsumed()))
        );
    }
    ...
    public static create(data: any): Consumer {
        ...
        switch (type) {
            ...
            case 'object':
                return new Consumer(
                    Object.fromEntries(Object.entries(data).map((_) => [_[0], Consumer.create(_[1])]))
                );
            case 'null':
        ...
        if (this.type === 'object') {
            return Object.fromEntries(
                Object.entries(this.data)
                    ...
                    .map(([key, value]) => [key, (value as Consumer).getMissingReport()])
            );
        }
        if (this.type === 'array')
