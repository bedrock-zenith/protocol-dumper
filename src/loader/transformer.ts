...
import { DDUIDynamicValue, NBTCComponent, VoidDataComponent } from './types/built-ins';
...
export class Transformer {
    ...
    public async dump(folder: string): Promise<void> {
        ...
        for (const file of this.byLayout.values()) {
            ...
            await writeFile(join(folder, file.name + '.json'), JSON.stringify(type, null, 2));
    ...
    public file(file: string): ReferenceComponent {
        ...
        if (!consumer.getIsConsumed())
            throw new ReferenceError(
                "Consumer wasn't properly consumed: " + JSON.stringify(consumer.getMissingReport())
            );

        return type;
    }

    public resolveDataComponent(context: Context, consumer: Consumer): BaseComponent {
        if (consumer.hasProperty<SchemaDefinition>('$ref')) {
            const file = consumer.getProperty<SchemaDefinition>('$ref').extract('string');
        ...
        if (consumer.hasProperty<SchemaDefinition>('type')) {
            const kind = consumer.getProperty<SchemaDefinition>('type').extract('string');

            if (consumer.hasProperty<SchemaDefinition>('enum')) return new EnumComponent();

            if (kind === 'object') {
                const additional = consumer.getProperty<SchemaDefinition>('additionalProperties');
                if (additional.hasValue()) {
                    if (
                        additional.getProperty<SchemaDefinition>('type').extractOptional('string') ===
                            'object' ||
            ...
            throw new TypeError(
                'Can not resolve type at the moment, type: ' + kind + ' at: ' + context.getPath()
            );
        }

        if (consumer.hasProperty<SchemaDefinition>('description')) {
            const description = consumer.getProperty<SchemaDefinition>('description').extract('string');
            if (description === 'Dynamic value') return new DDUIDynamicValue();

            throw new TypeError(
                'Unknown build in type, description: ' + description + ' at: ' + context.getPath()
            );
        }
        return new NBTCComponent();
    }

    public registerNewType(type: DataComponent, name: string): ReferenceComponent {
