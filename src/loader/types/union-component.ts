...
import { UnionEncodingComponent, type EncodingComponent } from './encoding-component';
...
export class UnionComponent extends DataComponent {
    ...
    public override getIdentityKey(): string {
        return super.getIdentityKey() + this.fields.map((_) => _.getIdentityKey());
    }
    ...
    public override process(context: Context, consumer: Consumer): void {
        ...
        for (const field of unionFields.getIterator()) {
            const type = context.child(String(fields.length), field, 'consumeWithEncoding');
