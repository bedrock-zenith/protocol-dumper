...
export abstract class NumberComponent<T extends Integer | Float> extends DataComponent {
    ...
    public override process(context: Context, consumer: Consumer): void {
        if (consumer.hasProperty<SchemaDefinition>('x-underlying-type')) {
            ...
            if (!n) throw new TypeError(`Unknown ${this.type} underlying type: ${underlying}`);
    ...
    public override getEncoding(): EncodingComponent | null {
        if (this.type === 'integer') return new IntegerEncodingComponent('integer', this.number as Integer);
