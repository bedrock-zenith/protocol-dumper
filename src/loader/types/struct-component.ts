...
import { StructEncodingComponent, type EncodingComponent } from './encoding-component';
...
export class StructComponent extends DataComponent {
    ...
    public override getIdentityKey(): string {
        return super.getIdentityKey() + this.fields.map((_) => _.getIdentityKey()).join('');
    }
    ...
    public override process(context: Context, consumer: Consumer): void {
        ...
        for (const key of keys) {
            ...
            const index = consumer.getProperty<SchemaField>('x-ordinal-index').extract('number');
...
export class FieldComponent extends BaseComponent {
    ...
    public override getFileContent(input: object): object {
        ...
        Reflect.set(input, 'field_type', this.child.getFileContent(Object.create(null)));
        return input;
    }
    public override getTypeContent(input: object): object {
        Reflect.set(input, 'field_name', this.name);
        Reflect.set(input, 'field_type', this.child.getTypeContent(Object.create(null)));
        return input;
    }
    public override getIdentityKey(): string {
