...
import { EnumEncodingComponent, type EncodingComponent } from './encoding-component';
...
export class EnumComponent extends DataComponent {
    ...
    public override process(context: Context, consumer: Consumer): void {
        ...
        const enumType = consumer.getProperty<SchemaDefinition>('type').extract('string');

        if (enumType !== 'string') throw new TypeError('Unknown enum type: ' + enumType);

        const underlying = consumer.getProperty<SchemaDefinition>('x-underlying-type').extract('string');
    ...
    public override getIdentityKey(): string {
        return super.getIdentityKey() + JSON.stringify(this.fields) + (this.backing?.getIdentityKey() ?? '');
    }
    public override getLayoutKey(): string {
        return super.getLayoutKey() + JSON.stringify(this.fields) + (this.backing?.getLayoutKey() ?? '');
    }
