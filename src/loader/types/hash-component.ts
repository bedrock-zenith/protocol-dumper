...
import { IntegerEncodingComponent, type EncodingComponent } from './encoding-component';
...
export class HashMapComponent extends DataComponent {
    ...
    public override process(context: Context, consumer: Consumer): void {
        ...
        if (pn.hasValue()) {
            ...
            if (ap.getProperty<SchemaDefinition>('type').extract('string') != 'object')
                throw new TypeError('Has to be object with key:value pair');
            const keyValue = ap.getProperty<SchemaDefinition>('properties');

            const key = context.child('#key', keyValue.getProperty('key'), 'consumeWithEncoding');

            const value = context.child('#value', keyValue.getProperty('value'), 'consumeWithEncoding');
